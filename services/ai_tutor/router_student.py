...
router = APIRouter(prefix="/tutor", tags=["student-tutor"])
...
def _ensure_owner(conv_student_id, user: TokenPayload) -> None:
    """确认当前用户是会话所有者"""
    if str(conv_student_id) != str(user.sub):
...
async def submit_feedback(
    ...
    logger.info("feedback_submitted", message_id=message_id, rating=body.rating)
