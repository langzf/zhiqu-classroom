...
class ContentService:
    ...
    async def update_textbook(
        ...
        *,
        ...
        if grade_range is not None:
            tb.grade_range = grade_range
        await self.db.flush()
        logger.info("textbook_updated", textbook_id=str(tb.id))
        return tb
    ...
    async def delete_textbook(self, textbook_id: str) -> Textbook:
        """soft_delete_textbook 的别名"""
        return await self.soft_delete_textbook(textbook_id)
    ...
    async def get_chapters(self, textbook_id: str) -> list[Chapter]:
        """get_chapter_tree 的别名，供 admin router 使用"""
        return await self.get_chapter_tree(textbook_id)
    ...
    async def get_exercises(
        ...
        """获取练习列表（resource_type='exercise' 的生成资源），支持分页"""
        base = GeneratedResource.resource_type == "exercise"
        filters = [base]
        if knowledge_point_id:
            filters.append(GeneratedResource.knowledge_point_id == knowledge_point_id)

        count_stmt = select(func.count()).select_from(GeneratedResource).where(*filters)
        total = (await self.db.execute(count_stmt)).scalar() or 0
    ...
    async def create_exercise(
        ...
        """手动创建练习（本质是 resource_type='exercise' 的 GeneratedResource）"""
        return await self.save_generated_resource(
